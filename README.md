...
- **예약 승인 / 거절** — 예약은 `승인 대기` 상태로 접수되고, 시간표 주인이 **승인해야 확정**됩니다. 대기 중인 예약은 점선으로 구분 표시
- **예약 일괄 정리** — 주인이 `🧹 예약 정리`로 대기 건만 또는 전체 예약을 한 번에 삭제 (다음 주 새로 받을 때 유용)
...
schedules/{uid}/reservations/{slotId}
   { day, hour, mark, note(공개), byUid, byName, byPhoto, isGuest,
     status: "pending" | "approved",   # 주인 승인 여부 (필드 없으면 구버전 = 승인됨)
     createdAt }
